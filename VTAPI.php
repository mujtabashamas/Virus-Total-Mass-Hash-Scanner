...
class VirusTotalAPIV2 {
	 ...
	 */
	public function displayResult($result, $hash) {
		$data = array();
		...
		if (!isset($result->response_code)) return $data;
		if ($result->response_code != 1) return $data;
		
		//print_r($result);
		if (count($result->scans)) {
        	echo "<table class='table table-condensed'>
    				...
    				<tbody>";
  			foreach ($result->scans as $idx => $scans) {
				if($idx === "McAfee" || $idx === "TrendMicro" || $idx === "Fortinet" || $idx === "F-Secure"){
					echo "<tr>";
	            	echo "<td>".$idx."</td>";
	            	echo "<td>$scans->version</td>";
	            	if(empty($scans->result)){
	            		echo "<td><i style='color:green' class='fa fa-thumbs-o-up' aria-hidden='true'></i><td>";
	            		if($idx === "McAfee")
	            			$data[1] = "None";
	            		else if($idx === "TrendMicro")
	            			$data[2] = "None";
	            		else if($idx === "Fortinet")
	            			$data[3] = "None";
	            		else if($idx === "F-Secure")
	            			$data[4] = "None";
	            	}
	            	else{
						echo "<td style='color:red' class='fa fa-thumbs-o-down'>  $scans->result</td>";
						if($idx === "McAfee")
	            			$data[1] = "Detected";
	            		else if($idx === "TrendMicro")
	            			$data[2] = "Detected";
